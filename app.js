...
function bubbleLayout(values,rect){
  const count=values.length;
  if(!count)return {radii:[],top:125};
  const top=Math.min(125,Math.max(72,rect.height*.22));
  const usableW=Math.max(1,rect.width-12),usableH=Math.max(1,rect.height-top-8);
  const min=Math.min(...values),max=Math.max(...values);
  const norms=values.map(v=>max===min?0.5:Math.sqrt(Math.max(0,(v-min)/(max-min))));
  const baseRadii=norms.map(norm=>42+norm*34);
  const baseArea=baseRadii.reduce((sum,r)=>sum+Math.PI*r*r,0);
  const availableArea=usableW*usableH;
  const targetFill=count<=6?.5:count<=12?.44:count<=24?.38:.34;
  const scale=Math.min(1,Math.sqrt((availableArea*targetFill)/Math.max(1,baseArea)));
  return {top,norms,radii:baseRadii.map(r=>Math.max(7,r*scale))};
}
function renderBubbles(){
  bodies=[];world.innerHTML='';
  const values=state.subscriptions.map(monthlyRub),rect=world.getBoundingClientRect();
  const {top,norms=[],radii=[]}=bubbleLayout(values,rect),usableH=Math.max(1,rect.height-top);
  state.subscriptions.forEach((s,i)=>{
    const norm=norms[i]??.5,r=radii[i]??42,el=document.createElement('div');
    el.className=`bubble${r<28?' bubble-compact':''}${r<19?' bubble-micro':''}`;
    el.dataset.id=s.id;
    el.style.cssText=`width:${r*2}px;height:${r*2}px;--bubble:${COLORS[i%COLORS.length]};--bubble-r:${r}px`;
    el.innerHTML=`<span class="bubble-name">${escapeHtml(s.name)}</span><strong class="bubble-price">${money(originalMonthly(s),s.currency,s.currency==='RUB'?0:2)}</strong><span class="bubble-period">в месяц</span>`;
    world.appendChild(el);
    const angle=i*2.399963229728653+(i%3)*.17,spread=Math.sqrt((i+.7)/Math.max(1,state.subscriptions.length));
    const maxRX=Math.max(0,rect.width*.46-r),maxRY=Math.max(0,usableH*.44-r);
    const x=rect.width/2+Math.cos(angle)*maxRX*spread;
    const y=top+usableH/2+Math.sin(angle)*maxRY*spread;
    bodies.push({id:s.id,el,r,mass:1+norm*5,x:Math.max(r,Math.min(rect.width-r,x)),y:Math.max(top+r,Math.min(rect.height-r,y)),vx:(Math.random()-.5)*38,vy:(Math.random()-.5)*38});
  });
  bindDrag();
}
