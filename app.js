...
const PERIODS={day:{label:'день',yearly:365},week:{label:'неделя',yearly:52},month:{label:'месяц',yearly:12},year:{label:'год',yearly:1}};
...
function rubAmount(s){return s.amount*(s.currency==='RUB'?1:(state.rates[s.currency]||1))}
function yearlyRub(s){return rubAmount(s)*(PERIODS[s.period]?.yearly||12)}
function monthlyRub(s){return yearlyRub(s)/12}
...
function periodLabel(period){return PERIODS[period]?.label||'месяц'}
...
function renderList(){const el=$('#subscription-list');if(!state.subscriptions.length){el.innerHTML='<div class="empty">Пока пусто.<br>Добавьте первую подписку.</div>';return}el.innerHTML=state.subscriptions.map((s,i)=>`<article class="list-item" data-id="${s.id}"><div class="category-dot" style="background:${COLORS[i%COLORS.length]}">${s.name.trim()[0]?.toUpperCase()||'•'}</div><div><div class="item-name">${escapeHtml(s.name)}</div><div class="item-meta">${escapeHtml(s.category||'Без категории')}${s.nextPayment?` · ${new Date(s.nextPayment+'T00:00').toLocaleDateString('ru-RU')}`:''}</div></div><div class="item-price">${money(s.amount,s.currency,s.currency==='RUB'?0:2)}<small>в ${periodLabel(s.period)}</small></div></article>`).join('');el.querySelectorAll('.list-item').forEach(x=>x.onclick=()=>openEditor(x.dataset.id))}
